...
from detectron2.lib.layers.wrappers import make_conv, make_norm
...
class FPN(nn.Module):
    ...
    def _make_layer(self, dim_in, fpn_dim, norm):
        # P5 in
        ...
        for i in range(self.num_backbone_stages - 1):  # skip the top layer
            px_in = make_conv(dim_in[-i - 2], fpn_dim, kernel_size=1, norm=make_norm(fpn_dim, norm=norm))  # P4 to P2
